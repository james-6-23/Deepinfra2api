...
# 显示标题
echo -e "${CYAN}🦕 DeepInfra2API Deno 版本部署脚本${NC}"
echo -e "${CYAN}========================================${NC}"
echo ""
...
check_port() {
    ...
    if netstat -tuln 2>/dev/null | grep -q ":${port} " || ss -tuln 2>/dev/null | grep -q ":${port} "; then
        ...
        if [[ ! "$continue_deploy" =~ ^[Yy]$ ]]; then
            echo -e "${BLUE}💡 请修改 .env 文件中的 DENO_PORT 配置${NC}"
            exit 1
        fi
    else
...
deploy() {
    ...
    if docker compose up -d --build; then
        ...
        echo ""
        echo -e "${BLUE}📋 服务信息:${NC}"
        echo -e "  🔗 访问地址: http://localhost:${DENO_PORT:-8000}"
        echo -e "  📊 健康检查: curl http://localhost:${DENO_PORT:-8000}/health"
        ...
        sleep 5
        if curl -f http://localhost:${DENO_PORT:-8000}/health >/dev/null 2>&1; then
        ...
        echo "  3. 检查端口占用: netstat -tuln | grep ${DENO_PORT:-8000}"
        exit 1
    fi
}
